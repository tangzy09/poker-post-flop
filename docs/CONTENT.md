...
测试覆盖：重复牌、听牌类型、label 与牌力、solver 牌面、learn key 注册、stem 与 spot 一致等。两个 `tools/*` 体检脚本**已收编进 `npm test`**（test/qa-tools.test.js），另有 test/seo-artifacts.test.js 锁死 slug↔课程页↔sitemap↔SEO 块的同步——忘跑生成器时 CI 直接红。
