'use strict';
/*
 * embed-fonts.js — 把 Space Grotesk 的 latin 子集 woff2 以 base64 内嵌进 index.html,
 * 替换原来的 Google Fonts CDN <link>(大陆被墙 + 断网时品牌字体失效)。
 * 中文字体 Noto Sans SC 不内嵌(体积过大;移动端/桌面系统自带等价 CJK 字体),
 * 仍保留在 font-family 栈里走系统回退。
 * 与 pokerPreFlop 的 tools/embed-fonts.js 同源同策略(两站字体交付统一)。
...
// 从 css2 输出里取「/* latin */」块的 woff2 URL(纯 latin 子集,覆盖 ASCII + Latin-1)
...
(async () => {
  ...
  const style =
`<!-- 内嵌字体(离线/大陆可用):Space Grotesk latin 子集;中文(Noto Sans SC)走系统回退 -->
  <style>
@font-face{font-family:'Space Grotesk';font-style:normal;font-weight:500 700;font-display:swap;src:url(data:font/woff2;base64,${grot.b64}) format('woff2')}
  ...
  if (start < 0 || cssLink < 0) throw new Error('找不到要替换的 <link> 行(可能已内嵌过)');
  ...
  console.log('done — index.html 已内嵌字体,CDN 引用已移除');
