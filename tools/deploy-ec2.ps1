...
scp -i $Key -o StrictHostKeyChecking=no "$Root\index.html" "$Root\privacy.html" "$Root\robots.txt" "$Root\sitemap.xml" "$Root\og-image.png" "$Root\google8d5971246131b6a2.html" "${Remote}:${Dest}/"
scp -i $Key -o StrictHostKeyChecking=no -r "$Root\js" "$Root\data" "$Root\courses" "${Remote}:${Dest}/"
